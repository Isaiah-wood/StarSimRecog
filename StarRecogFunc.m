...
function binaryImg = Binarization(srcImg, mode, arg1, arg2)
    ...
    % if strcmpi(mode, 'RegionMeanBased') && nargin < 4
    %     arg2 = 5;
    % end
...
function starList = CenterExtraction(srcImg, binaryImg, method, bgdThres, sizeThres)
    ...
    if strcmp(method, 'withthres') && (nargin < 4 || length(bgdThres) == 0)
        bgdThres = imgMean;
    end
    if nargin < 5 || length(sizeThres) == 0
        sizeThres = 1;
    end
